subroutine complex_ppRLR(TDA,nOO,nVV,Bpp,Cpp,Dpp,Om1,X1,Y1,Om2,X2,Y2,EcRPA)
  ...
  complex*16,intent(in)         :: Bpp(nVV,nOO),Cpp(nVV,nVV),Dpp(nOO,nOO)
  complex*16,intent(out)        :: Om1(nVV),X1(nVV,nVV),Y1(nOO,nVV)
  complex*16,intent(out)        :: Om2(nOO),X2(nVV,nOO),Y2(nOO,nOO)
  complex*16,intent(out)        :: EcRPA
  
  logical                       :: imp_bio,verbose
  integer                       :: i,j
  integer                       :: Npp
  complex*16                    :: EcRPA1,EcRPA2
  ...
  if(TDA) then
    ...
    X1(:,:) = +Cpp(:,:)
    Y1(:,:) = 0d0
    if(nVV > 0) call complex_diagonalize_matrix(nVV,X1,Om1)

    X2(:,:) = 0d0
    Y2(:,:) = -Dpp(:,:)
    if(nOO > 0) call complex_diagonalize_matrix(nOO,Y2,Om2)
