...
!-------------------------------------!
  ...
  !-------------------------------------!
    ...
    !$OMP PARALLEL DEFAULT(NONE)    &                                       
    !$OMP PRIVATE(p,j,n,num,dem,reg,num1,num2) & 
    !$OMP SHARED(nC,nO,nOrb,nR,nS,eta,eQP,eh_sing_rho,eh_sing_Om,int3,SigC,Z)  
    !$OMP DO
    do p=nC+1,nOrb-nR
      do j=nC+1,nO
        do n=1,nS
 
          num1 = eh_sing_rho(p,j,n) + int3(j,p,n)
          num2 = eh_sing_rho(j,p,n) + int3(p,j,n)
  ...
  !-------------------------------------!
    ...
    !$OMP PARALLEL DEFAULT(NONE)    &                                       
    !$OMP PRIVATE(p,i,a,j,n,num,dem,reg,num1,num2) & 
        ...
        do i=nC+1,nO
          do p=nC+1,nOrb-nR
            ...
            do n=1,nS
              ...
              num1 = eh_sing_rho(a,i,n)
              num2 = eh_sing_rho(j,p,n)
              dem = eQP(a) - eQP(i) + eh_sing_Om(n)
              reg = (1d0 - exp(- 2d0 * eta * dem * dem))/dem
 
              int4(p,j,i,a) = int4(p,j,i,a) + num1 * num2 * reg
 
              num1 = eh_sing_rho(i,a,n)
              num2 = eh_sing_rho(p,j,n)
              dem = eQP(a) - eQP(i) - eh_sing_Om(n)
              reg = (1d0 - exp(- 2d0 * eta * dem * dem))/dem
    ...
    !$OMP PARALLEL DEFAULT(NONE)    &                                       
    !$OMP PRIVATE(p,i,j,a,num,dem,reg,num1,num2) & 
    !$OMP SHARED(nC,nO,nOrb,nR,nS,eta,eQP,ERI,int4,SigC,Z)  
    !$OMP DO
    do p=nC+1,nOrb-nR
      do i=nC+1,nO
        do j=nC+1,nO
          do a=nO+1,nOrb-nR
            ...
            num1 = 0.5d0*ERI(p,a,i,j) - ERI(p,a,j,i) + int4(j,p,i,a)
            num2 = 0.5d0*ERI(j,a,i,p) - ERI(j,a,p,i) + int4(p,j,i,a)
  ...
  !-------------------------------------!
    ...
    !$OMP PARALLEL DEFAULT(NONE)    &                                       
    !$OMP PRIVATE(p,b,n,num,dem,reg,num1,num2) & 
    !$OMP SHARED(nC,nO,nOrb,nR,nS,eta,eQP,eh_sing_rho,eh_sing_Om,int3,SigC,Z)  
    !$OMP DO
    do p=nC+1,nOrb-nR
      do b=nO+1,nOrb-nR
        do n=1,nS
          ...
          num2 = eh_sing_rho(p,b,n) + int3(b,p,n)
  ...
  !-------------------------------------!
    ...
    !$OMP PARALLEL DEFAULT(NONE)    &                                       
    !$OMP PRIVATE(p,i,a,b,num,dem,reg,num1,num2) & 
    !$OMP SHARED(nC,nO,nOrb,nR,nS,eta,eQP,ERI,int4,SigC,Z)  
    !$OMP DO
    do p=nC+1,nOrb-nR
      do i=nC+1,nO
        do a=nO+1,nOrb-nR
          do b=nO+1,nOrb-nR
            ...
            num1 = 0.5d0*ERI(p,i,a,b) - ERI(p,i,b,a) + int4(p,b,i,a)
            num2 = 0.5d0*ERI(b,i,a,p) - ERI(b,i,p,a) + int4(b,p,i,a)
            dem = eQP(p) + eQP(i) - eQP(a) - eQP(b)
            reg = (1d0 - exp(- 2d0 * eta * dem * dem))/dem
...
!-------------------------------------!
  ...
  !-------------------------------------!
    ...
    !$OMP SHARED(nC,nO,nOrb,nR,nS,eta,eQP,eh_trip_rho,eh_trip_Om,int3,SigC,Z)  
    !$OMP DO
    do p=nC+1,nOrb-nR
      do j=nC+1,nO
        do n=1,nS
...
!-------------------------------------!
  ...
  !-------------------------------------!
    ...
    !$OMP SHARED(nC,nO,nOrb,nR,nVVs,eta,eQP,ee_sing_rho,ee_sing_Om,int3,SigC,Z)  
    !$OMP DO
    do p=nC+1,nOrb-nR
      do k=nC+1,nO
        do n=1,nVVs
  ...
  !-------------------------------------!
    ...
    !$OMP PARALLEL DEFAULT(NONE)    &                                       
    !$OMP PRIVATE(p,i,j,a,num,dem,reg) & 
    !$OMP SHARED(nC,nO,nOrb,nR,eta,eQP,ERI,int4,SigC,Z)  
    !$OMP DO
    do p=nC+1,nOrb-nR
      do i=nC+1,nO
        do j=nC+1,nO
  ...
  !-------------------------------------!
    ...
    !$OMP SHARED(nC,nO,nOrb,nR,nOOs,eta,eQP,hh_sing_rho,hh_sing_Om,int3,SigC,Z)  
    !$OMP DO
    do p=nC+1,nOrb-nR
      do c=nO+1,nOrb-nR
        do n=1,nOOs
...
!-------------------------------------!
  ...
  !-------------------------------------!
    ...
    !$OMP PARALLEL DEFAULT(NONE)    &                                       
    !$OMP PRIVATE(p,k,n,num,dem,reg) & 
    !$OMP SHARED(nC,nO,nOrb,nR,nVVt,eta,eQP,ee_trip_rho,ee_trip_Om,int3,SigC,Z)  
    !$OMP DO
    do p=nC+1,nOrb-nR
      do k=nC+1,nO
        do n=1,nVVt
  ...
  !-------------------------------------!
    ...
    !$OMP PARALLEL DEFAULT(NONE)    &                                       
    !$OMP PRIVATE(p,c,n,num,dem,reg) & 
    !$OMP SHARED(nC,nO,nOrb,nR,nOOt,eta,eQP,hh_trip_rho,hh_trip_Om,int3,SigC,Z)  
    !$OMP DO
    do p=nC+1,nOrb-nR
      do c=nO+1,nOrb-nR
        do n=1,nOOt
