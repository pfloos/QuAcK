subroutine MOM_cUHF(dotest,maxSCF,thresh,max_diis,guess_type,mix,level_shift,writeMOs,nNuc,ZNuc,rNuc,ENuc, &
             nBas,nO,S,T,V,ERI,CAP,X,EUHF,eHF,c,P,F,occupationsGuess,working_dir)
...
!   Level-shifting
    ...
    if(level_shift > 0d0 .and. Conv > thresh) then
      print *, "No level shift implemented for this method !"
    end if
...
!   Compute density matrix 
    ...
    do ispin=1,nspin
      call complex_MOM_density_matrix(nBas, nBas, nO(ispin), S, c(:,:,ispin), cGuess(:,:,ispin), &
