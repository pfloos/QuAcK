subroutine MOM_GHF(dotest,maxSCF,thresh,max_diis,guess_type,mix,level_shift,writeMOs,nNuc,ZNuc,rNuc,ENuc, & 
               nBas,nBas2,nO,Ov,T,V,Hc,ERI,dipole_int,Or,EGHF,eHF,c,P,F,occupationsGuess,working_dir)
...
! Memory allocation
  ...
  allocate(cGuess(nBas2,nBas2),occupations(nO)) 
...
!   Form super density matrix
    call MOM_density_matrix(nBas2, nBas2, nO, S, c, cGuess, &
