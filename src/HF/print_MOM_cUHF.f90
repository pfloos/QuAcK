subroutine print_MOM_cUHF(nBas,nO,S,eHF,c,ENuc,ET,EV,EJ,Ex,EW,EUHF,occupations)
...
allocate(unoccupied(nBas - minval(nO),nspin))
  ...
  Sx2 = real(0.25d0*dble(nO(1) - nO(2)) + 0.5d0*nO(2) - &
        0.5d0*sum(matmul(transpose(conjg(c(:,occupations(1:nO(1),1),1))),&
...
! Print results
  ...
  write(*,'(A40)') '---------------------------------------'
  call complex_vecout(nO(1),eHF(occupations(1:nO(1),1),1))
  write(*,*)
  write(*,'(A40)') '---------------------------------------'
  write(*,'(A40)') ' UHF Unoccupied alpha orbital energies '
  write(*,'(A40)') '---------------------------------------'
  call complex_vecout(nBas - nO(1),eHF(unoccupied(1:nBas - nO(1),1),1))
  write(*,*)
  write(*,'(A40)') '---------------------------------------'
  write(*,'(A40)') ' UHF Occupied beta orbital energies  '
  write(*,'(A40)') '---------------------------------------'
  call complex_vecout(nO(2),eHF(occupations(1:nO(2),2),2))
  write(*,*)
  write(*,'(A40)') '---------------------------------------'
  write(*,'(A40)') ' UHF Unoccupied beta orbital energies '
  write(*,'(A40)') '---------------------------------------'
  call complex_vecout(nBas - nO(2),eHF(unoccupied(1:nBas-nO(2),2),2))
  write(*,*)
