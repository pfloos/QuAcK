subroutine print_MOM_ROHF(nBas, nOrb, nO, eHF, c, ENuc, ET, EV, EJ, Ex, EROHF, dipole, occupations)

! Print MOM occupations and orbital energies + usual ROHF output
...
! Print results
  ...
  if(nD >0) then 
    ...
    write(*,'(A50)') '--------------------------------------------'
    call vecout(nD, eHF(doublyOccupied(1:nD)))
    write(*,*)
  end if
  if(nSA > 0) then
    write(*,'(A50)') '------------------------------------------------'
    write(*,'(A50)') ' Only alpha occupied ROHF orbital energies (au) '
    write(*,'(A50)') '------------------------------------------------'
    call vecout(nSA, eHF(singlyOccupiedA(1:nSA)))
    write(*,*)
  end if
  if(nSB > 0) then
    write(*,'(A50)') '-----------------------------------------------'
    write(*,'(A50)') ' Only beta occupied ROHF orbital energies (au) '
    write(*,'(A50)') '-----------------------------------------------'
    call vecout(nSB, eHF(singlyOccupiedB(1:nSB)))
    write(*,*)
  end if
  if(nV > 0) then
    write(*,'(A50)') '------------------------------------'
    write(*,'(A50)') ' Virtual ROHF orbital energies (au) '
    write(*,'(A50)') '------------------------------------'
    call vecout(nV, eHF(virtual(1:nV)))
    write(*,*)
  end if
