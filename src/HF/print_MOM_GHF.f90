subroutine print_MOM_GHF(nBas,nBas2,nO,eHF,C,S,ENuc,ET,EV,EJ,EK,EGHF,dipole,occupations)
...
! Density matrices
  ...
  Ca(:,:) = C(     1:nBas ,occupations(1:nO))
  Cb(:,:) = C(nBas+1:nBas2,occupations(1:nO))
...
! Print results
  ...
  write(*,'(A50)') '---------------------------------------'
  write(*,'(A50)') ' GHF occupied orbital energies (au) '
  write(*,'(A50)') '---------------------------------------'
