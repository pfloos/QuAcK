subroutine GGW_phBSE_omegazero(dophBSE2,TDA_W,TDA,dBSE,dTDA,eta,nBas,nC,nO,nV,nR,nS,ERI,dipole_int,eW,eGW,EcBSE)
...
!---------------------------------
  ...
  call GGW_phBSE_static_kernel_A_omegazero(eta,nBas,nC,nO,nV,nR,nS,1d0,ERI,eGW,OmRPA,rho_RPA,KA_sta)
...
end subroutine GGW_phBSE_omegazero

subroutine GGW_phBSE_static_kernel_A_omegazero(eta,nBas,nC,nO,nV,nR,nS,lambda,ERI,eGW,Om,rho,KA)
      ...
      do j=nC+1,nO
        do b=nO+1,nBas-nR
          ...
          do kc=1,nS
            ...
            dem = eGW(b) - eGW(i) + Om(kc)
            KA(ia,jb) = KA(ia,jb) + num*dem/(dem**2 + eta**2)
             
            dem = eGW(a) - eGW(j) + Om(kc)
            KA(ia,jb) = KA(ia,jb) + num*dem/(dem**2 + eta**2)
