subroutine complex_complex_RGW_SRG_self_energy_diag(flow,eta,nBas,nOrb,nC,nO,nV,nR,nS,Re_e,Im_e,Om,rho,EcGM,Re_Sig,Im_Sig,Re_Z,Im_Z)
  ...
  do p=nC+1,nBas-nR
    do i=nC+1,nO
      do m=1,nS
        ...
        num = 2d0*abs(rho(p,i,m))**2*(1d0 - exp(-2d0*s*(eps**2 + eta_tilde**2)))
  ...
  do p=nC+1,nBas-nR
    do a=nO+1,nBas-nR
      do m=1,nS
        ...
        num = 2d0*abs(rho(p,a,m))**2*(1d0 - exp(-2d0*s*(eps**2 + eta_tilde**2)))
        tmp = num*cmplx(eps/(eps**2 + eta_tilde**2),&
                -eta_tilde/(eps**2 + eta_tilde**2),kind=8)
        Re_Sig(p) = Re_Sig(p) + real(tmp)
        Im_Sig(p) = Im_Sig(p) + aimag(tmp)
