subroutine MOM_UG0W0(dotest,doACFDT,exchange_kernel,doXBS,dophBSE,TDA_W,TDA,dBSE,dTDA,spin_conserved,spin_flip, &
...
! Spin-conserving transitions
  ...
  isp_W = 1
                 ...
                 call MOM_phULR_A(isp_W,dRPA_W,nBas,nC,nO,nV,nR,nSa,nSb,nSt,nCVS,nFC,occupations_fc,virtuals,1d0,eHF,ERI_aaaa,ERI_aabb,ERI_bbbb,Aph)
  if(.not.TDA_W) call MOM_phULR_B(isp_W,dRPA_W,nBas,nC,nO,nV,nR,nSa,nSb,nSt,nCVS,nFC,occupations_fc,virtuals,1d0,ERI_aaaa,ERI_aabb,ERI_bbbb,Bph)

  call MOM_phULR(TDA_W,nSa,nSb,nSt,Aph,Bph,EcRPA,Om,XpY,XmY)
...
!----------------------!
  ...
  call MOM_UGW_excitation_density(nBas,nC,nO,nR,nSa,nSb,nSt,nCVS,nFC,occupations_fc,virtuals,ERI_aaaa,ERI_aabb,ERI_bbbb,XpY,rho)
...
!------------------------------------------------!
  ...
  if(doSRG) then
    call MOM_UGW_SRG_self_energy_diag(flow,nBas,nC,nO,nV,nR,nSt,nCVS,nFC,occupations_fc,virtuals,eHF,Om,rho,EcGM,SigC,Z)
  else
    call MOM_UGW_self_energy_diag(eta,nBas,nC,nO,nV,nR,nSt,nCVS,nFC,occupations_fc,virtuals,eHF,Om,rho,EcGM,SigC,Z)
  end if
...
!-----------------------------------!
  ...
  ! Find graphical solution of the QP equation
    ...
    do is=1,nspin
      ...
      call MOM_UGW_QP_graph(doSRG,eta,flow,nBas,nC(is),nO(is),nV(is),nR(is),nSt,nCVS(is),nFC(is),occupations_fc(:,is),virtuals(:,is),eHF(:,is), & 
...
! Compute RPA correlation energy
                 ...
                 call MOM_phULR_A(isp_W,dRPA_W,nBas,nC,nO,nV,nR,nSa,nSb,nSt,nCVS,nFC,occupations_fc,virtuals,1d0,eGW,ERI_aaaa,ERI_aabb,ERI_bbbb,Aph)
  if(.not.TDA_W) call MOM_phULR_B(isp_W,dRPA_W,nBas,nC,nO,nV,nR,nSa,nSb,nSt,nCVS,nFC,occupations_fc,virtuals,1d0,ERI_aaaa,ERI_aabb,ERI_bbbb,Bph)
...
! Perform BSE calculation
  ...
  if(dophBSE) then
    ...
    call MOM_UGW_phBSE(exchange_kernel,TDA_W,TDA,dBSE,dTDA,spin_conserved,spin_flip,eta,nBas,nC,nO,nV,nR,nS, &
