subroutine MOM_evUGW(dotest,maxSCF,thresh,max_diis,doACFDT,exchange_kernel,doXBS,BSE,TDA_W,TDA,dBSE,dTDA, & 
...
!------------------------------------------------------------------------
  ...
  do while(Conv > thresh .and. nSCF <= maxSCF)
    ...
    if(.not.TDA) call MOM_phULR_B(ispin,dRPA,nBas,nC,nO,nV,nR,nSa,nSb,nSt,nCVS,nFC,occupations_fc,virtuals,1d0,ERI_aaaa,ERI_aabb,ERI_bbbb,Bph)
    ...
    if(doSRG) then
      call MOM_UGW_SRG_self_energy_diag(flow,nBas,nC,nO,nV,nR,nSt,nCVS,nFC,occupations_fc,virtuals,eGW,Om,rho,EcGM,SigC,Z)
    else
      call MOM_UGW_self_energy_diag(eta,nBas,nC,nO,nV,nR,nSt,nCVS,nFC,occupations_fc,virtuals,eGW,Om,rho,EcGM,SigC,Z)
    end if
