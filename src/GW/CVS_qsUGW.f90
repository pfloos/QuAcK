subroutine CVS_qsUGW(dotest,maxSCF,thresh,max_diis,doACFDT,exchange_kernel,doXBS,dophBSE,TDA_W,TDA,dBSE,dTDA,spin_conserved,spin_flip, &
...
!------------------------------------------------------------------------
  ...
  do while(Conv > thresh .and. nSCF < maxSCF)
    ...
    if(.not. TDA_W) call CVS_phULR_B(ispin,dRPA,nBas,nC,nO,nV,nR,nSa,nSb,nSt,nCVS,nFC,occupations_fc,virtuals,1d0,ERI_aaaa,ERI_aabb,ERI_bbbb,Bph)
    ...
    if(doSRG) then
      call CVS_UGW_SRG_self_energy(flow,nBas,nC,nO,nV,nR,nSt,nCVS,nFC,occupations_fc,virtuals,eGW,Om,rho,EcGM,SigC,Z)
    else
      call CVS_UGW_self_energy(eta,nBas,nC,nO,nV,nR,nSt,nCVS,nFC,occupations_fc,virtuals,eGW,Om,rho,EcGM,SigC,Z)
    end if
