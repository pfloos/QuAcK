subroutine R_psdG3W2_plot_self_energy(nOrb,nC,nO,nV,nR,nS,eHF,eQP,Om,rho,ERI)
...
! Occupied part of the self-energy and renormalization factor
  ...
  do g=1,nGrid
    do p=nC+1,nOrb-nR
      ...
      call R_psdG3W2_self_energy_iieta(p,w(g),eta,nOrb,nC,nO,nV,nR,nS,eHF,Om,rho,ERI,tmp_Re_SigC,tmp_Im_SigC,tmp_Re_dSigC)
...
! Compute spectral function
  ...
  do g=1,nGrid
    do p=nC+1,nOrb-nR
      A(p,g) = ImSigC(p,g)/((w(g) - eHF(p) - ReSigC(p,g))**2 + ImSigC(p,g)**2)
    end do
  end do
...
! Dump quantities in files as a function of w
  ...
  open(unit=8 ,file='R_psdG3W2_ReSigC.dat')
  open(unit=9 ,file='R_psdG3W2_freq.dat')
  open(unit=10 ,file='R_psdG3W2_Z.dat')
  open(unit=11 ,file='R_psdG3W2_A.dat')
  open(unit=12 ,file='R_psdG3W2_ImSigC.dat')
