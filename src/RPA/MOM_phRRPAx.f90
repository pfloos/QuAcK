subroutine MOM_phRRPAx(dotest,TDA,doACFDT,exchange_kernel,singlet,triplet,nBas,nC,nO,nV,nR,nS,nCVS,FC,occupations,ENuc,ERHF,ERI,dipole_int,eHF)
...
! Singlet manifold
  ...
  if(singlet) then 
    ...
    ispin = 1
                 ...
                 call MOM_phRLR_A(ispin,dRPA,nBas,nC,nO,nV,nR,nSCVS,nCVS,nFC,occupations_fc,virtuals,lambda,eHF,ERI,Aph)
    if(.not.TDA) call MOM_phRLR_B(ispin,dRPA,nBas,nC,nO,nV,nR,nSCVS,nCVS,nFC,occupations_fc,virtuals,lambda,ERI,Bph)

    call MOM_phRLR(TDA,nSCVS,Aph,Bph,EcRPA(ispin),Om,XpY,XmY)
    call print_excitation_energies('phRPAx@RHF','singlet',nSCVS,Om)
    call MOM_phLR_transition_vectors(.true.,nBas,nC,nO,nV,nR,nSCVS,nCVS,nFC,occupations_fc,virtuals,dipole_int,Om,XpY,XmY)
...
! Triplet manifold 
  ...
  if(triplet) then 
    ...
    call MOM_phLR_transition_vectors(.false.,nBas,nC,nO,nV,nR,nSCVS,nCVS,nFC,occupations_fc,virtuals,dipole_int,Om,XpY,XmY)
