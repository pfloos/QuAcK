subroutine quadDyson_GW_RHFB(nBas,nOrb,nOrb_twice,c,eQP_state,nfreqs,wweight,wcoord,ERI,vMAT,U_QP,&
                            Enuc,EcGM,sigma,T,V,S,P,Panom,Pcorr,Panomcorr)

! Use the restricted Sigma_c(E) to compute the quadratic approx. to the Dyson eq
...
! Local variables
  ...
  double precision              :: ET,EV,EJ,EK,EL,EquadG,trace_occ,N_anom,trace_r_can,dev_Idemp_r_can
...
! Allocate and initialize arrays and variables
  ...
  & Go^Gorkov Sigma^Gorkov Go^Gorkov Sigma^Gorkov Go^Gorkov *'
  write(*,*) '*                               Bogoliubov quadratic-Dyson &
...
! Integration along imag. freqs contributions
  ...
  do ifreq=1,nfreqs2
   ...
   Tmp_QP2(:,:)=matmul(Tmp_QP(:,:),matmul(Sigma_c_QP(:,:),Tmp_QP(:,:)))  ! This is G^Gorkov(iw2) Sigma_c^Gorkov(iw2) G^Gorkov(iw2)
   Tmp_QP(:,:)=Tmp_QP2(:,:)                                             &
              +matmul(Tmp_QP2(:,:),matmul(Sigma_c_QP(:,:),Tmp_QP(:,:)))  ! This is G^Gorkov(iw2) Sigma_c^Gorkov(iw2) G^Gorkov(iw2) + quad
...
! Print results
  ...
  write(*,'(A33,1X,F16.10,A3)') ' B-quad-Dyson energy = ',EquadG + ENuc,' au'
  write(*,'(A50)')           '---------------------------------------'
  ...
  write(*,'(A50)')           '---------------------------------------'
  write(*,'(A50)') '-----------------------------------------'
  write(*,'(A50)') ' Bogoliubov quad-Dyson occupation numbers '
  write(*,'(A50)') '-----------------------------------------'
  trace_occ=0d0
  do iorb=1,nOrb
  ...
  deallocate(Rcorr,Sigma_c_QP,Tmp_QP2,Tmp_QP,Tmp_mo,Pcorr_mo,Panomcorr_mo,wweight2,wcoord2,wcoord2_cpx)
