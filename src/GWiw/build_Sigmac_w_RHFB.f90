  ...
  do ifreq=1,nfreqs
   ...
   ! Use Wp (iw) to build all Sigma_c(E)
    do iE=1,nE
     ...
     if(doSigc_eh) then
      weval=wtest(iE)+im*wcoord(ifreq)
      call G_MO_HFB(nOrb,nOrb_twice,eta,eHFB,weval,Mat2,Mat2,Mat1,Mat1,G_mo_5)
      weval=wtest(iE)-im*wcoord(ifreq)
      call G_MO_HFB(nOrb,nOrb_twice,eta,eHFB,weval,Mat2,Mat2,Mat1,Mat1,G_mo_6)
     endif
     ! Gee
     if(doSigc_ee) then
      weval=wtest(iE)+im*wcoord(ifreq)
      call G_MO_HFB(nOrb,nOrb_twice,eta,eHFB,weval,Mat2,Mat1,Mat1,-Mat2,G_mo_7)
      weval=wtest(iE)-im*wcoord(ifreq)
      call G_MO_HFB(nOrb,nOrb_twice,eta,eHFB,weval,Mat2,Mat1,Mat1,-Mat2,G_mo_8)
     endif
     ! Sigma_c(E)
     do iorb=1,nOrb
      do jorb=1,nOrb
