subroutine cubDyson_GW_RHF(nBas,nOrb,nO,c,eHF,nfreqs,wweight,wcoord,ERI,vMAT,&
                         Enuc,EcGM,T,V,S,P,Pcorr)


! Use the restricted Sigma_c(E) to compute the cubic approximation to G
...
! Local variables
  ...
  double precision              :: ET,EV,EJ,EK,EcubG,trace_occ
  ...
  complex *16,allocatable       :: Tmp_mo2(:,:)
  complex *16,allocatable       :: Tmp_mo3(:,:)
...
! Allocate and initialize arrays and variables
  ...
  write(*,*)
  write(*,*) '*********************************************************************************'
  write(*,*) '*  G = Go + Go Sigma Go + Go Sigma Go Sigma Go + Go Sigma Go Sigma Go Sigma Go  *'
  write(*,*) '*                     cubic-Dyson equation approximation                        *'
  write(*,*) '*********************************************************************************'
  write(*,*)
  ...
  allocate(Tmp_mo2(nOrb,nOrb),Tmp_mo3(nOrb,nOrb))
...
! Integration along imag. freqs contributions
  ...
  do ifreq=1,nfreqs2
   ...
   call G_MO_RHF(nOrb,nO,0d0,eHF,wcoord2_cpx(ifreq),Tmp_mo)                 ! G(iw2)
   Tmp_mo2(:,:)=matmul(Tmp_mo(:,:),matmul(Sigma_c(ifreq,:,:),Tmp_mo(:,:)))  ! G(iw2) Sigma_c(iw2) G(iw2)
   Tmp_mo3(:,:)=matmul(Tmp_mo2(:,:),matmul(Sigma_c(ifreq,:,:),Tmp_mo(:,:))) ! G(iw2) Sigma_c(iw2) G(iw2) Sigma_c(iw2) G(iw2)
   Tmp_mo(:,:)=Tmp_mo2(:,:)+Tmp_mo3(:,:)                                    &
              +matmul(Tmp_mo3(:,:),matmul(Sigma_c(ifreq,:,:),Tmp_mo(:,:)))  ! G(iw2) Sigma_c(iw2) G(iw2) + quad + cub
...
! Print results
  ...
  write(*,'(A33,1X,F16.10,A3)') ' cub-Dyson    energy = ',EcubG + ENuc,' au'
  write(*,'(A50)')           '---------------------------------------'
  write(*,'(A50)') '---------------------------------------'
  write(*,'(A50)') ' cub-Dyson occupation numbers '
  write(*,'(A50)') '---------------------------------------'
  trace_occ=0d0
  do iorb=1,nOrb
