subroutine quadDyson_GW_RHF(nBas,nOrb,nO,c,eHF,nfreqs,wweight,wcoord,ERI,vMAT,&
                         Enuc,EcGM,T,V,S,P,Pcorr)


! Use the restricted Sigma_c(E) to compute the quadratic approximation to G
...
! Local variables
  ...
  double precision              :: ET,EV,EJ,EK,EquadG,trace_occ
...
! Allocate and initialize arrays and variables
  ...
  write(*,*)
  write(*,*) '*************************************************'
  write(*,*) '*  G = Go + Go Sigma Go + Go Sigma Go Sigma Go  *'
  write(*,*) '*    quadratic-Dyson equation approximation     *'
  write(*,*) '*************************************************'
  write(*,*)
  ...
  allocate(Tmp_mo(nOrb,nOrb),Tmp_mo2(nOrb,nOrb),Pcorr_mo(nOrb,nOrb))
...
! Integration along imag. freqs contributions
  ...
  do ifreq=1,nfreqs2
   ...
   Tmp_mo(:,:)=Tmp_mo2(:,:)                                                &
              +matmul(Tmp_mo2(:,:),matmul(Sigma_c(ifreq,:,:),Tmp_mo(:,:)))  ! G(iw2) Sigma_c(iw2) G(iw2) +  G(iw2) Sigma_c(iw2) G(iw2) Sigma_c(iw2) G(iw2) 
...
! Print results
  ...
  write(*,'(A50)')           '---------------------------------------'
  write(*,'(A33,1X,F16.10,A3)') ' Electronic   energy = ',EquadG,' au'
  write(*,'(A33,1X,F16.10,A3)') ' Nuclear   repulsion = ',ENuc,' au'
  write(*,'(A33,1X,F16.10,A3)') ' quad-Dyson   energy = ',EquadG + ENuc,' au'
  write(*,'(A50)')           '---------------------------------------'
  write(*,'(A50)') '---------------------------------------'
  write(*,'(A50)') ' quad-Dyson occupation numbers '
  write(*,'(A50)') '---------------------------------------'
  trace_occ=0d0
  do iorb=1,nOrb
  ...
  deallocate(Sigma_c,Tmp_mo,Tmp_mo2,Pcorr_mo,wweight2,wcoord2,wcoord2_cpx)
