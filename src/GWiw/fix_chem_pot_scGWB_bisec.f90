subroutine fix_chem_pot_scGWB_bisec(iter_hfb,nBas,nBas_twice,nfreqs,nElectrons,thrs_N,thrs_Ngrad,chem_pot,S,H_ao_hfb,Sigma_c_w_ao,wcoord,wweight, &
                                    G_ao,G_ao_iw_hfb,DeltaG_ao_iw,R_ao,R_ao_hfb,trace_1_rdm,chem_pot_hf,verbose) 
...
! Output variables
  ...
  if(verbose) then
   write(*,*)
   write(*,'(a,i5)') ' Fixing the Tr[1D] at scGW/scGF2 at Fock iter ',iter_hfb
   write(*,*)
  endif
  call get_1rdm_scGWB(nBas,nBas_twice,nfreqs,chem_pot,S,H_ao_hfb,Sigma_c_w_ao, &
                      wcoord,wweight,G_ao,G_ao_iw_hfb,DeltaG_ao_iw,R_ao,R_ao_hfb,trace_old) 
  if(verbose) then
   write(*,*)'------------------------------------------------------'
  ...
  do while( abs(trace_old-nElectrons) > thrs_closer .and. isteps <= 100 )
   ...
   call get_1rdm_scGWB(nBas,nBas_twice,nfreqs,chem_pot,S,H_ao_hfb,Sigma_c_w_ao, &
                       wcoord,wweight,G_ao,G_ao_iw_hfb,DeltaG_ao_iw,R_ao,R_ao_hfb,trace_old) 
   call get_1rdm_scGWB(nBas,nBas_twice,nfreqs,chem_pot-delta_chem_pot,S,H_ao_hfb,Sigma_c_w_ao, &
                       wcoord,wweight,G_ao,G_ao_iw_hfb,DeltaG_ao_iw,R_ao,R_ao_hfb,trace_down) 
   call get_1rdm_scGWB(nBas,nBas_twice,nfreqs,chem_pot+delta_chem_pot,S,H_ao_hfb,Sigma_c_w_ao, &
                       wcoord,wweight,G_ao,G_ao_iw_hfb,DeltaG_ao_iw,R_ao,R_ao_hfb,trace_up) 
...
!  write(*,*) "| fiding bisection method bounds ...                  |"
  delta_chem_pot  = 2d-2
  call get_1rdm_scGWB(nBas,nBas_twice,nfreqs,chem_pot,S,H_ao_hfb,Sigma_c_w_ao, &
                      wcoord,wweight,G_ao,G_ao_iw_hfb,DeltaG_ao_iw,R_ao,R_ao_hfb,trace_1_rdm)
  isteps = 1
  chem_pot_up   = chem_pot + delta_chem_pot
  chem_pot_down = chem_pot - delta_chem_pot
  do
   call get_1rdm_scGWB(nBas,nBas_twice,nfreqs,chem_pot_up,S,H_ao_hfb,Sigma_c_w_ao, &
                       wcoord,wweight,G_ao,G_ao_iw_hfb,DeltaG_ao_iw,R_ao,R_ao_hfb,trace_up)
   call get_1rdm_scGWB(nBas,nBas_twice,nfreqs,chem_pot_down,S,H_ao_hfb,Sigma_c_w_ao, &
                       wcoord,wweight,G_ao,G_ao_iw_hfb,DeltaG_ao_iw,R_ao,R_ao_hfb,trace_down)
