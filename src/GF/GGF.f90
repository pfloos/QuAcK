subroutine GGF(dotest,doG0F2,doevGF2,doqsGF2,doG0F3,dopsdG0F3,maxSCF,thresh,max_diis,dophBSE,doppBSE,TDA,dBSE,dTDA,linearize,eta,regularize, & 
...
!------------------------------------------------------------------------
  ...
  if(doG0F3) then
    ...
    t_GF = end_GF - start_GF
    write(*,'(A65,1X,F9.3,A8)') 'Total CPU time for G0F3 = ',t_GF,' seconds'
    write(*,*)
...
!------------------------------------------------------------------------
  ...
  if(dopsdG0F3) then
    ...
    call wall_time(start_GF)
    call GpsdG0F3(dotest,linearize,eta,regularize,nBas,nC,nO,nV,nR,nS,ENuc,EHF,ERI,epsHF)
    call wall_time(end_GF)

    t_GF = end_GF - start_GF
    write(*,'(A65,1X,F9.3,A8)') 'Total CPU time for psdG0F3 = ',t_GF,' seconds'
    write(*,*)
