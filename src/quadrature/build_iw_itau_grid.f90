...
!------------------------------------------------------------------------
 ...
 eEXP(1)=eHF(nO+1)
 eEXP(2)=max(abs(eHF(1)),abs(eHF(nOrb)))
 ...
 write(*,'(a,i20)')     '  Nfreqs max                  : ',nfreqs_max
 write(*,*)
  ...
  do
     ...
     if(interval_todo(iinterval)) then
      ...
      do ifreq=1,ngrid
       ...
       do itau=1,n01
        ...
        G_2vals(1)=G_2vals(1)-im*tweight_eval*im*Exp(teval*eHF(nO))           *Exp(-im*teval*wcoord(ifreq)) !  Go(i tau)
        G_2vals(2)=G_2vals(2)-im*tweight_eval*im*Exp(teval*(eHF(1)-eHF(nOrb)))*Exp(-im*teval*wcoord(ifreq)) ! ~Xo(i tau)
       enddo
       interval_vals(ifreq,1)=G_2vals(1)
       interval_vals(ifreq,2)=G_2vals(2)
      enddo
      ...
      do ifreq=1,ngrid
       ...
       do itau=1,n01
        ...
        G_2vals(2)=G_2vals(2)-im*tweight_eval*im*Exp(teval*(eHF(1)-eHF(nOrb)))*Exp(-im*teval*wcoord(ifreq)) ! ~Xo(i tau)
       enddo
       G_2vals(:)=G_2vals(:)-interval_vals(ifreq,:)
       err_G_set=err_G_set+abs(G_2vals(1))+abs(G_2vals(2))
      enddo
   ...
   do ifreq=1,ngrid
    ...
    do itau=1,ntimes                        ! This is Go or ~Xo 
     G_2vals(1)=G_2vals(1)-im*tweight(itau)*im*Exp(tcoord(itau)*eHF(nO))           *Exp(-im*tcoord(itau)*wcoord(ifreq)) !  Go(i tau)
     G_2vals(2)=G_2vals(2)-im*tweight(itau)*im*Exp(tcoord(itau)*(eHF(1)-eHF(nOrb)))*Exp(-im*tcoord(itau)*wcoord(ifreq)) ! ~Xo(i tau)
    enddo
    G_2vals(1)=G_2vals(1)-1d0/(im*wcoord(ifreq)-eHF(nO))
    G_2vals(2)=G_2vals(2)-1d0/(im*wcoord(ifreq)-(eHF(1)-eHF(nOrb)))
    err_G_set=err_G_set+abs(G_2vals(1))+abs(G_2vals(2))
   enddo
...
!------------------------------!
 ...
 if(nfreqs==0) then
  ...
  niter_max=40
  max_err_G_set=1d-3
  ! Build the 40 trial times from 0 to max_teval
  allocate(tcoord(ngrid))
  tcoord(1)=min_teval
  tcoord(ngrid)=max_teval
  do itau=2,ngrid-1
   call random_number(ran_num)
   ran_num=ran_num*max_teval
   tcoord(itau)=ran_num
  enddo
  call sort_ascending(ngrid,tcoord)
  tcoord=-tcoord ! From -0.0002 to -max_teval
  ...
  do    
   ...
   if(iter>0) then
    ...
    do iinterval=1,nintervals ! ith interval
     if(interval_todo(iinterval)) then
      ! For the ith active interval [a,b] compute all G_interval(itau_k) = i/(2 pi) int _a ^b G(i w) Exp(i w tau_k)  + Conjg[ G(i w) Exp(i w tau_k) ] d w
      !                                                                  = i/pi int _a ^b Re[G(i w)] Cos(w tau_k) - Im[G(i w)] Sin(w tau_k) d w
      interval_vals=czero
      do itau=1,ngrid
       G_2vals=czero
       do ifreq=1,n01
        ...
        G_2vals(1)=G_2vals(1)+wweight_eval*integrand_w2t(weval,tcoord(itau),eEXP(1)) ! Go(i w, smallest |Ep|)
        G_2vals(2)=G_2vals(2)+wweight_eval*integrand_w2t(weval,tcoord(itau),eEXP(2)) ! Go(i w,  largest |Ep|)
       enddo
       interval_vals(itau,1)=G_2vals(1)
       interval_vals(itau,2)=G_2vals(2)
      enddo
      ! Divide the ith interval in halves and compute coords and weights
      interval_r2(2*iinterval-1,1)=interval_r(iinterval,1)
      interval_r2(2*iinterval-1,2)=interval_r(iinterval,1)+0.5d0*(interval_r(iinterval,2)-interval_r(iinterval,1))
      ...
      interval_tmp_weight2(2*iinterval  ,:)=weight_01(:)*(interval_r2(2*iinterval  ,2)-interval_r2(2*iinterval  ,1))
      interval_tmp_coord2(2*iinterval  ,:) = coord_01(:)*(interval_r2(2*iinterval  ,2)-interval_r2(2*iinterval  ,1))+interval_r2(2*iinterval  ,1)
      ! Check the difference in G_interval(itau_k) computed as [a,b] vs [a,0.5(b-a)+a] + [0.5(b-a)+a,b]
      err_G_set=0d0
      do itau=1,ngrid
       G_2vals=czero
       do ifreq=1,n01
        ...
        G_2vals(2)=G_2vals(2)+wweight_eval*integrand_w2t(weval,tcoord(itau),eEXP(2)) ! Go(i w,  largest |Ep|)
       enddo
       G_2vals(:)=G_2vals(:)-interval_vals(itau,:)
       err_G_set=err_G_set+abs(G_2vals(1))+abs(G_2vals(2))
      enddo
      ! If we entered here, we always retain the new 2 halves of the original interval
      ! If diff. is small we keep the small intervals, but deactivate these two intervals
   ...
   ! Transfer from the matrices to the vectors wweight and wcoord to compute the full Fourier transform (i.e., from -Infty to Infty)
   !    G(itau_k) = i/pi int _0 ^Infty Re[G(i w)] Cos(w tau_k) - Im[G(i w)] Sin(w tau_k) d w
   ...
   do itau=1,ngrid
    G_2vals=czero
    do ifreq=1,nfreqs 
     G_2vals(1)=G_2vals(1)+wweight(ifreq)*integrand_w2t(wcoord(ifreq),tcoord(itau),eEXP(1)) ! Go(i w, smallest |Ep|)
     G_2vals(2)=G_2vals(2)+wweight(ifreq)*integrand_w2t(wcoord(ifreq),tcoord(itau),eEXP(2)) ! Go(i w,  largest |Ep|)
    enddo
    G_2vals(1)=G_2vals(1)-(-im*Exp(tcoord(itau)*eEXP(1)))
    G_2vals(2)=G_2vals(2)-(-im*Exp(tcoord(itau)*eEXP(2)))
    err_G_set=err_G_set+abs(G_2vals(1))+abs(G_2vals(2))
   enddo
   err_G_set=err_G_set/(2*ngrid) ! MAE
   write(*,'(a,f8.4,a,i5,a,i8,a)') ' MAE in reproducing G(itau) [Integrated - Analytic]',err_G_set,' at iter ',iter,' with ',nfreqs,' grid'
   if(iter>0) then
    if(abs(err_G_set_old-err_G_set)<1d-8 .or. nintervals_old==nintervals) then
     write(*,'(a,e15.8,a,e15.8)') ' Reducing the MAE accepted for each interval integrations form ',thrs_interval,' to ',1d-1*thrs_interval
     thrs_interval=1d-1*thrs_interval
...
function integrand_w2t(w,t,ep) result(val)

  implicit none
  include 'parameters.h'

  double precision,intent(in)  :: w,t,ep
  complex*16                   :: val

  val=(w*sin(w*t)-ep*cos(w*t)) / (ep*ep+w*w)
  val=im*val/pi
